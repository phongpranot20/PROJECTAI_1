...
export async function getChatResponseStream(message: string, history: any[] = [], imageBase64?: string, mimeType?: string, lang: 'th' | 'en' = 'th') {
  ...
  for (const model of AVAILABLE_MODELS) {
    try {
      ...
      lastError = error;
      console.warn(`โมเดล ${model} เกิดข้อผิดพลาด หรือโควต้าเต็ม กำลังลองตัวถัดไป...`);
      continue; // ข้ามไปตัวถัดไปเงียบๆ ตามที่ผู้ใช้ต้องการ
    }
  }
...
export async function getChatResponse(message: string, history: any[] = [], imageBase64?: string, mimeType?: string) {
  ...
  for (const model of AVAILABLE_MODELS) {
    try {
      ...
      lastError = error;
      console.warn(`โมเดล ${model} (Non-stream) เกิดข้อผิดพลาด กำลังลองตัวถัดไป...`);
      continue;
    }
  }
...
function handleGeminiError(error: any) {
  ...
  const lastUpdate = "4 เม.ย. 2569 - 22:18 (UTC)"; // เวลาอัปเดตล่าสุด
